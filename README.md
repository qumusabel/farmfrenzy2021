...
Бинарный файл doker-proxy используется как reverse shell в зараженную систему.
...
Более надежное – фильтровать то, что присылается по порту 1443, если оно начинается на байты ```\xcd\x0c``` – поднять тревогу
